...
// Renders an SVG string safely (our own generated markup).
function Figure({ svg, className = "" }) {
  return (
    <span
...
export default function NonVerbalBank({ meta, sections }) {
  ...
  return (
    <div>
          ...
          <div className="ml-auto">
            <button
              onClick={() => {
                ...
                if (!showAnswers) {
                  setTimeout(() => {
                    document.getElementById("nv-answer-key")?.scrollIntoView({ behavior: "smooth" });
                  }, 50);
                }
              }}
        ...
        {visible.map((sec) => (
          <section key={sec.id}>
            ...
            <div className="mt-4 space-y-5">
              {sec.items.map((item) => (
                <div key={item.no} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  {/* Question text */}
                  <div className="flex gap-2">
                    ...
                    <div>
                      ...
                      {item.hi && <p className="mt-1 text-sm text-slate-500">{item.hi}</p>}
                    </div>
                  </div>
                  ...
                  {item.figure && (
                    <div className="mt-3 flex items-center gap-3">
                      <span className="text-xs font-semibold text-slate-400">Question figure:</span>
                      <Figure svg={item.figure} className="h-24 w-24" />
                    </div>
                  )}

                  {/* Options: figures OR text */}
                  {item.optFigures ? (
                    <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {item.optFigures.map((svg, i) => {
                        ...
                        return (
                          <div
                            ...
                            className={
                              ...
                              (correct ? "border-green-500 bg-green-50" : "border-slate-200 bg-slate-50")
                            }
                          >
                            <div className="mb-1 text-xs font-bold text-slate-400">
                              {String.fromCharCode(65 + i)} {correct && "✓"}
                            </div>
                            <Figure svg={svg} className="h-20 w-20" />
                          </div>
                        );
                      })}
                    </div>
      ...
      {/* Answer key */}
      <section id="nv-answer-key" className="mt-14 scroll-mt-24">
        <div className="rounded-2xl border-2 border-dashed border-accent-500/50 bg-accent-500/5 p-6">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Sabhi {meta.total} questions ke sahi jawaab. (Detailed explanation ke liye upar "Show
            Answer Key" dabao.)
          </p>
